...
class FakeNavigationContextManager:
    ...
    def __enter__(self):
        ...
        return FakeNavigationInfo(self.page)
...
def test_navigation_result_value_belongs_only_to_entered_event_info():
    page = FakePage()
    navigation_context = page.expect_navigation()
    assert not hasattr(navigation_context, "value")
    with navigation_context as navigation_info:
        assert hasattr(navigation_info, "value")
        page.click("#submit")
    assert navigation_info.value is not None
